...
import {
  ...
  AlertCircle, AlertTriangle, Loader2, Building2, Users, Bell, UserPlus, Trash2, Shield, Eye, Edit, Crown, X, DollarSign, ChevronDown, MessageSquare, ShoppingCart, Check, CheckCheck,
} from 'lucide-react';
...
export function SettingsContent() {
  ...
  const getNotifIcon = (type: Notification['type']) => {
    ...
    switch ((type as string).toLowerCase()) {
      case 'success': return <CheckCircle className={`${cls} text-[var(--success)]`} />;
      case 'error': return <XCircle className={`${cls} text-[var(--danger)]`} />;
      case 'warning': return <AlertTriangle className={`${cls} text-[var(--warning)]`} />;
      case 'order': return <ShoppingCart className={`${cls} text-[var(--info)]`} />;
      default: return <Bell className={`${cls} text-[var(--info)]`} />;
    }
  };
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <h4 className={cn('font-semibold text-sm', n.read ? 'text-[var(--text-secondary)]' : 'text-[var(--text-primary)]')}>
                          {n.title}
                        </h4>
                      ...
                      {n.action_label && n.action_url && (
                        <a href={n.action_url} className="inline-block mt-2 text-sm text-[var(--primary)] font-medium hover:underline">
                          {n.action_label} &rarr;
                        </a>
                      )}
