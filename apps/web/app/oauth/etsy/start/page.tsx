  ...
  useEffect(() => {
    ...
    const startOAuth = async () => {
      try {
        ...
        // Validate first
        const validateRes = await fetch(`${baseUrl}/api/shops/connect-link/${linkToken}/validate`)
        if (!validateRes.ok) {
        ...
        const startRes = await fetch(`${baseUrl}/api/shops/connect-link/${linkToken}/start`, { method: 'POST' })
        if (!startRes.ok) {
      ...
      <div className="max-w-md w-full mx-4 p-8 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl text-center">
        {status === 'validating' && (
          <>
            ...
            <h1 className="text-xl font-semibold text-[var(--text-primary)] mb-2">Validating Connection Link</h1>
            <p className="text-[var(--text-muted)] text-sm">Please wait while we verify your link...</p>
          </>
        )}
        {status === 'redirecting' && (
          <>
            <CheckCircle className="w-12 h-12 text-green-500 mx-auto mb-4" />
            <h1 className="text-xl font-semibold text-[var(--text-primary)] mb-2">Redirecting to Etsy</h1>
            <p className="text-[var(--text-muted)] text-sm">You will be redirected to approve the connection...</p>
          </>
        )}
        {status === 'error' && (
          <>
